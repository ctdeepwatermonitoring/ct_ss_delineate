...
try:
    ...
    if connection.is_connected():
        ...
        sites = data[data['summer_category'] == 'Cold']
        #print(cold_sites)
        ...
        # Get staSeqs from the filtered DataFrame and query the database for their coordinates
        staSeqs = tuple(sites['staSeq'].tolist())
        cursor = connection.cursor()
        if staSeqs:
            placeholders = ','.join(['%s'] * len(staSeqs))
            query = f"SELECT ylat, xlong, staSeq FROM stations WHERE staSeq IN ({placeholders})"
            cursor.execute(query, staSeqs)
            results = cursor.fetchall()
            #results_df = pd.DataFrame(results, columns=['ylat', 'xlong', 'staSeq', 'locationName'])
            #results_df.to_csv("abm_24-25_cold_sites_results.csv", index=False)  # Save results to CSV
            ...
            for ylat, xlong, staSeq in results:
